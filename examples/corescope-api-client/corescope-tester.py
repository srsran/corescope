...
class coreScopeTester:
    ...
    def switch_on(self, ue_id):
        ue_status_update_args = dto_ue_status_update.DtoUeStatusUpdate(
            switch_on=True)
    ...
    def add_ue(self):
        ...
        ue_args.usim = dto_ue_args.DtoUeUsimArgs(
            imsi=imsi, imei=imei, opc=opc, k=k
        )
        ...
        add_ue_data: Response[dto_ue_id.DtoUeId] = create_ue.sync_detailed(
            client=self.client, gnb_id=0, json_body=ue_args)
    ...
    def is_connected(self, ue_id):
        ue_status_data: Response[dto_ue_status.T] = get_ue_status.sync_detailed(
            client=self.client, gnb_id=0, ue_id=ue_id)
        if not str(ue_status_data.status_code).startswith("2"):
            print("Getting UE status failed. Exiting... (%s)" %
                  ue_status_data.status_code)
            exit(1)
        else:
...
ping_proc = Popen(["ping", corenetwork_ip, "-I",
                   "tun_ue_{}".format(ue_id), "-c", "3"])
